...
async def _ensure_control_bot():
    global _control_bot_task
    if not CONTROL_BOT_ENABLED or not BOT_TOKEN:
        return
    if _control_bot_task is None or _control_bot_task.done():
        _control_bot_task = asyncio.create_task(_control_bot_loop())
        log.info("Control bot task started (lazy)")
